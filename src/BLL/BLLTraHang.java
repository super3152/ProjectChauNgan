...
public class BLLTraHang {
    ...
    public static double TongTienTPT;
    ...
    public static int HienThiChiTietTraHang(JTable tbl, int MaTraHang) {
        ...
        try {
            while (rs.next()) {
                ...
                double PhiTra = rs.getDouble("phitra");
                double TongTienTPTra = ThanhTien + PhiTra;
                obj[10] = ChuyenDoi.DinhDangTien(TongTienTPTra);
                tbModel.addRow(obj);
                int soluong = 0;
                for (int i = 0; i < tbModel.getRowCount(); i++) {
                    soluong = soluong + Integer.parseInt(tbl.getValueAt(i, 5).toString());
                    SoLuong = soluong;
                    double tientpt = BLL.ChuyenDoi.ChuyenTien(tbl.getValueAt(i, 10).toString());
                    TongTienTPT = tientpt;
                }
    ...
    public static void HienThiTraHangLoc(JTable tbl, String MaKhachHang, String MaNhanVien, String TrangThaii, String HoanTienn, int Trang) {
        ResultSet rs = DAO.DAOTraHang.LayTraHangLoc(MaKhachHang, MaNhanVien, TrangThaii, HoanTienn, Trang);
        DefaultTableModel tbModel = (DefaultTableModel) tbl.getModel();
        tbModel.setRowCount(0);
        ...
        try {
            while (rs.next()) {
                ...
                if (rsSP.next()) {
                    ...
                    GiaLe = rsSP.getDouble("giabanle");
                    obj[6] = ChuyenDoi.DinhDangTien(rsSP.getDouble("giabanle"));
                }
                ...
                obj[9] = rs.getInt("idsanpham");
                double ThanhTienTPT = GiaLe * SoLuong - PhiTra;
                obj[10] = ChuyenDoi.DinhDangTien(ThanhTienTPT);
    ...
    public static double HienThiChiTietHoaDonSua(JTable tbl, int MaCTHD, int SoLuong, double PhiTra) {
        ...
        try {
            while (rs.next()) {
                ...
                obj[9] = rs.getInt("idsanpham");
                double ThanhTienTPT = GiaLe * SoLuong;
                obj[10] = ChuyenDoi.DinhDangTien(ThanhTienTPT);
                tbModel.addRow(obj);
                for (int i = 0; i < tbModel.getRowCount(); i++) {
                    double tien = BLL.ChuyenDoi.ChuyenTien(tbl.getValueAt(i, 8).toString());
