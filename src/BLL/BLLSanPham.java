...
public static boolean KiemTraThemSanPham() {
      myList.clear();
      ...
      for(int i=0; i < tblThemThuocTinh.getRowCount(); i++) {
         ...
         if(value.trim().length() == 0) {    
               ThongBaoLoi.ThongBao("Vui lòng nhập đầy đủ thuộc tính", "Thông báo");
               return false;
            }
         }
      }
      
      if (pnlsanpham.tblThemThuocTinh.getRowCount() == 0) {
                  ThongBaoLoi.ThongBao("Thuộc tính sản phẩm trống"+"\n"+" Sản phẩm cần ít nhất 1 thuộc tính", "Thông báo");
                   return false;
            }
        int row = pnlsanpham.tblThemThuocTinh.getRowCount();   
     for (int i = 0; i < row; i++) {
         String a = pnlsanpham.tblThemThuocTinh.getValueAt(i, 1).toString();
          String b = pnlsanpham.tblThemThuocTinh.getValueAt(i, 2).toString();
         if (a.equals("Chọn size") || b.equals("Chọn màu")) {
               ThongBaoLoi.ThongBao("Thuộc tính sản phẩm trống", "Thông báo");
               return false;
         }
     }
         int row2 = pnlsanpham.tblThemThuocTinh.getRowCount();
                for (int c = 0; c < row2; c++) {
           String az = pnlsanpham.tblThemThuocTinh.getValueAt(c, 1).toString();
            String bz = pnlsanpham.tblThemThuocTinh.getValueAt(c, 2).toString();   
            String abz = (az+" - "+bz);
           myList.add(abz);
            for(int k =0; k < myList.size(); k++) {
  if (myList.lastIndexOf(myList.get(k)) != k)  {
    
     ThongBaoLoi.ThongBao("Thuộc tính "+myList.get(k)+" bị trùng", "Thông báo");
